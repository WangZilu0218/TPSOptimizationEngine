...
void __global__ projectionKernel(float *p_v, float lambda, bool pos, int size) {
  int idx = blockDim.x * blockIdx.x + threadIdx.x;
  while (idx < size) {
	p_v[idx] = projL1(p_v[idx], lambda, pos);
	idx += gridDim.x * blockDim.x;
  }
}

void __global__ absKernel(float *p_v, int size) {
  int idx = blockDim.x * blockIdx.x + threadIdx.x;
  while (idx < size) {
	p_v[idx] = fabsf(p_v[idx]);
	idx += gridDim.x * blockDim.x;
  }
}
...
void absVec(float *p_v, int size) {
  absKernel<<<GRIDDIM, BLOCKDIM>>>(p_v, size);
}
...
void projection(float *p_v, float lambda, bool pos, int size) {
  projectionKernel<<<GRIDDIM, BLOCKDIM>>>(p_v, lambda, pos, size);
}
