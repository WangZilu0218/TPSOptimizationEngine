...
void __global__ backwardUpperKernel(float *d_dose,
									float *d_dose_grad,
									float sum_dose_pow,
									float target,
									float a,
  ...
  while (idx < size) {
	...
	if (sign * (__powf(sum_dose_pow / size, 1 / a) - target) >= 0)
	  temp = 1;
	d_dose_grad[idx] = temp * (__powf(sum_dose_pow / size, 1 / a) - target)
		* (__powf(sum_dose_pow / size, 1 / a - 1) * __powf(d_dose[idx], a - 1) / size);
